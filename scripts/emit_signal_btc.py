...
def _news_score(hours=72, path="ping-pongings/knowledge-base/log_btc.jsonl"):
    ...
    with open(path) as f:
        for line in f:
            ...
            if not e.get("distilled"):
                continue
            d = e.get("distill", {})
            w = float(d.get("btc_relevance", 0) or 0)
            if w <= 0:
                continue
            try:
...
def main():
    ...
    if headlines:
        signal_text = "Recent BTC-relevant headlines: " + " | ".join(headlines)
    else:
        signal_text = os.getenv("BARROT_BTC_SIGNAL_TEXT", "market sideways; low conviction")
    ...
    with open("web/latest_signal_btc.json", "w", encoding="utf-8") as f:
    ...
    try:
        ...
        price_now = float(_pr.json().get("bitcoin", {}).get("usd", 0))
    except Exception:
        price_now = 0.0
    hist_entry = dict(out)
    hist_entry["price_at_emission"] = price_now
    with open("web/signal_history_btc.jsonl", "a", encoding="utf-8") as hf:
