#!/usr/bin/env python3
"""
BARROT-Ω KNOWLEDGE INGESTION — real, minimal, honest.
Fetches real RSS, distills each item, appends to knowledge-base/log.jsonl,
increments the source counter in config.json. Every entry has a real URL.
"""
...
CFG_PATH = os.path.join(KB_DIR, "config.json")

FEEDS = {
    "online_articles": [
        "https://cointelegraph.com/rss/tag/xrp",
        "https://news.google.com/rss/search?q=XRP+ripple+when:7d",
...
def parse_rss(raw, source, feed_url, limit=8):
    ...
    for item in root.iter("item"):
        ...
        out.append({
            ...
            "source": source, "feed": feed_url,
            "title": title[:300], "url": link,
            "published": pub, "summary": desc[:500],
        })
...
def main():
    ...
    for e in new[:5]: print(f"  - [{e['source']}] {e['title'][:70]}")
