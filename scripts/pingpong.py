...
CHAIN = [
    ("qwen/qwen3-32b", "DRAFT",
     "Produce a first substantive answer. Be concrete and specific."),
...
FINAL = ("llama-3.3-70b-versatile", "BARROT",
...
def run_chain(task):
    ...
    try:
        final = ask(model, instruction, f"TASK:\n{task}\n\nREFINED OUTPUT FROM THE CHAIN:\n{current}")
...
def log_cycle(task, final, stages):
    ...
    entry = {
        ...
        "final": final[:2000],
        "distilled": True,
        "distill": {"sentiment": "neutral", "catalyst": "ping_pong_refinement",
                    "xrp_relevance": 0.0,
