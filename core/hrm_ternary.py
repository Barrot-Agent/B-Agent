...
class Ternary:
    ...
    def resolve(*signals: float) -> int:
        ...
        if s >  ANCHOR * n: return Ternary.BUY
        if s < -ANCHOR * n: return Ternary.SELL
        return Ternary.NULL
...
def hrm_resolve(signals: dict) -> HRMResolution:
    ...
    k      = len(u_raw)
    if k == 0:
        return HRMResolution(0, "NULL", 0, 0.0, 0.0, 0.0, 0, False, False,
                             "no input sources")
    ...
    if k > 1 and bool(np.all(u_raw <= -ANCHOR)):
        return HRMResolution(0, "NULL", raw, 0.0, 1.0, 0.0, 0, False, True,
                             "sovereign absolution: unanimous SELL absorbed")

    # -- H/L nested convergence (HRM dynamics) ----------------------
    u   = np.zeros(16); u[:k] = u_raw
    z_L = np.zeros(DIM)
    z_H = np.zeros(DIM)
    drive = _W_IN @ u
    ...
    return HRMResolution(
        state=gated, label=Ternary.label(gated), raw_state=raw,
        confidence=confidence, agreement=round(agreement, 3),
        convergence=round(convergence, 3), iterations=steps,
        halted_early=halted, absolution_fired=False,
...
if __name__ == "__main__":
    ...
    for name, sig in cases.items():
        r = hrm_resolve(sig)
        print(f"{name:<18} {r.label:<5} {Ternary.label(r.raw_state):<5} "
