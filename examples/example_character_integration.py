...
def discover_capabilities_for_research(research_domain: str):
    ...
    print(
        f"Analyzing {len(all_characters)} characters across {len(db.get_all_genres())} genres...\n"
    )
    ...
    transformed_sorted = sorted(
        transformed, key=lambda x: impact_order.get(x["implementation"]["impact"], 0), reverse=True
    )
    ...
    for item in all_capabilities[:5]:  # Top 5 for example
        aug = CapabilityPermutator.augment_for_research(item["capability"], research_domain)
    ...
    if physical_caps and mental_caps:
        combo1 = CapabilityPermutator.combine_capabilities(
            physical_caps[0]["capability"], mental_caps[0]["capability"]
        )
    ...
    if magical_caps and mental_caps and len(mental_caps) > 1:
        combo2 = CapabilityPermutator.combine_capabilities(
            magical_caps[0]["capability"], mental_caps[1]["capability"]
        )
    ...
    for category, count in sorted(
        matrix["capability_categories"].items(), key=lambda x: x[1], reverse=True
    ):
        bar = "█" * (count // 2)
    ...
    for level, count in sorted(
        matrix["power_distribution"].items(), key=lambda x: x[1], reverse=True
    ):
        bar = "█" * (count // 2)
...
╚═══════════════════════════════════════════════════════════════════════════════╝
    ...
    research_domains = ["agi_development", "data_processing", "system_optimization"]
