...
def example_8_export_and_integration():
    ...
    sorted_categories = sorted(
        report["capabilities_by_category"].items(), key=lambda x: x[1], reverse=True
    )
...
def example_9_search_by_feature():
    ...
    desired_features = ["distributed", "prediction", "optimization", "intelligence"]
    ...
    for example in examples:
        try:
            ...
            print(
                ...
                ", ".join(list(create_character_database().characters.keys())[:5]) + ", ...",
            )
