...
class CoinAppAutomation:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.earnings = {"today": 0, "week": 0, "month": 0, "all_time": 0}
        self.activity_log = []
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "automation": {
                "frequency": "hourly",
                "max_daily_time": 120,
                "priority_order": ["high_reward_surveys", "nearby_geocaches", "limited_time_games"],
            },
        }
    ...
    def _prioritize_opportunities(self, opportunities: List[Dict]) -> List[Dict]:
        ...
        filtered = [opp for opp in opportunities if opp["estimated_reward"] >= min_threshold]

        # Sort by priority score (reward/time)
        sorted_opps = sorted(filtered, key=lambda x: x.get("priority_score", 0), reverse=True)
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["normal", "aggressive"], default="normal", help="Automation mode"
    )
    parser.add_argument(
        "--report", action="store_true", help="Generate and display earnings report"
    )
