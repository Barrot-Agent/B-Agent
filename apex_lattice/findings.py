...
persists them to .apex_lattice/findings/<timestamp>_<category>_<id>.json.
"""
...
class Finding:
    ...
    id: str
    cycle_id: str
    category: str  # e.g. "code", "security", "performance", etc.
    title: str
    description: str
...
class FindingGenerator:
    ...
    def load_all_ever(self) -> list[Finding]:
        """Load every persisted finding across all cycles (used by refinement loop)."""
        findings: list[Finding] = []
        for p in sorted(self._findings_dir.glob("*.json")):
            try:
                with p.open(encoding="utf-8") as fh:
                    data = json.load(fh)
                findings.append(Finding(**data))
