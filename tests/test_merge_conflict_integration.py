...
>>>>>>> feature/auth
    ...
    for i, resolution in enumerate(resolutions, 1):
        ...
        outcome = LearningOutcome(
            ...
            strategy_used=(
                ResolutionStrategy.AUTO_MERGE.value
                if not manual_required
                else ResolutionStrategy.MANUAL_MERGE.value
            ),
            success=success,
            ...
            improvements_suggested=[
                (
                    ...
                    if manual_required
                    else "Keep using automated resolution"
                )
            ],
        )
...
"""
    for i, resolution in enumerate(resolutions, 1):
        status = (
            "✅ Auto-resolved" if not outcomes[i - 1].manual_intervention_required else "👤 Manual"
        )
