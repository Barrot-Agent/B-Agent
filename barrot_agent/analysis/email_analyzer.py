...
class EmailAnalyzer:
    ...
    def _calculate_priority(
        self, content: str, sender: str, attachments: List[str]
    ) -> EmailPriority:
    ...
    def _extract_learning_content(
        ...
        for pattern in self.knowledge_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                learning_items.append(
                    {"type": "technical_content", "source": "email_body", "relevance": "high"}
                )
                break
        ...
        for attachment in attachments:
            if any(attachment.lower().endswith(ext) for ext in educational_extensions):
                learning_items.append(
                    {"type": "educational_document", "source": attachment, "relevance": "medium"}
                )
    ...
    def _generate_batch_summary(
        self, total: int, useful: int, high_priority: int, action_items: int, opportunities: int
    ) -> str:
