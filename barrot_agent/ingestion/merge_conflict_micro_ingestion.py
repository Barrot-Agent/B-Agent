...
>>>>>>> feature/data-analysis
    ...
    def _load_best_practices(self) -> None:
        ...
        self.best_practices = [
            ...
            BestPractice(
                ...
                examples=["Rerere automatically resolves repeated conflicts during rebases"],
                anti_patterns=[
    ...
    def get_recommended_strategy(
        self, conflict_type: str, file_path: str
    ) -> Optional[ResolutionTechnique]:
        """Get recommended resolution strategy based on conflict type and context"""
        applicable_techniques = [
            tech for tech in self.resolution_techniques if conflict_type in tech.applicable_types
        ]
    ...
    def analyze_conflict(self, conflict_content: str, file_path: str) -> Dict[str, Any]:
        ...
        if analysis["conflict_type"]:
            technique = self.get_recommended_strategy(analysis["conflict_type"], file_path)
            if technique:
    ...
    def export_to_json(self, output_dir: str = ".") -> Dict[str, str]:
        ...
        with open(patterns_file, "w") as f:
            json.dump([asdict(p) for p in self.conflict_patterns], f, indent=2)
        ...
        with open(practices_file, "w") as f:
            json.dump([asdict(p) for p in self.best_practices], f, indent=2)
        ...
        with open(outcomes_file, "w") as f:
            json.dump([asdict(o) for o in self.learning_outcomes], f, indent=2)
        ...
        summary = {
            ...
            "strategy_success_rates": {
                k: sum(v) / len(v) if v else 0.0 for k, v in self.strategy_success_rates.items()
            },
        }
